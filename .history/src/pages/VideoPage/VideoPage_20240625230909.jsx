...
import { collection, getDocs } from 'firebase/firestore'; // Firestoreから必要なものをインポート
import { firestore } from '../../firebase'; // Firestoreを初期化したファイルからインポート
...
const VideoPage = ({ relatedVideos, loading }) => {
    ...
    const [matchDocuments, setMatchDocuments] = useState([]); // Firestoreのドキュメントを保持するstate
    ...
    useEffect(() => {
        const fetchMatchDocuments = async () => {
            const matchDocs = [];
            const querySnapshot = await getDocs(collection(firestore, 'matches')); // matchesコレクションからドキュメントを取得
            querySnapshot.forEach((doc) => {
                matchDocs.push(doc.data());
            });
            setMatchDocuments(matchDocs); // 取得したドキュメントをstateにセット
        };
