...
const VideoPage = ({ relatedVideos, loading }) => {
    ...
    const [matchDocuments, setMatchDocuments] = useState([]);
    ...
    useEffect(() => {
        const fetchMatchDocuments = async() => {
            const matchDocs = [];
            const querySnapshot = await getDoc(collection(firestore, 'matches'));
    ...
    return (
        {/* サイトが開くまでの間(loading)と動画が開くまでの間(videosLoading)が両方開くまで待機する */}
