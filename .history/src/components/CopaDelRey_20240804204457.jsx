...
const CopaDelRey = ({ matches, loading }) => {
    ...
    useEffect(() => {
        ...
        const fillterdMatches = matches.filter(match => match.category === 'コパ・デル・レイ');
    ...
    return (
        ...
        <div className='copa-home'>
            <Navbar handleCopaDelReyClick={handleCopaDelReyClick} />
            <div className='copa-content'>
                <h2 className='copa-title'>コパ・デル・レイ</h2>
                <div className='copa-grid-container'>
                    {currentMatches.length > 0 ? currentMatches.map((match) => (
                        <Link key={match.id} to={`/video/${match.id}`} className='copa-grid-item' data-date={match.date}>
